...
public class LargeInt
{
  ...
  public static LargeInt multiply(LargeInt first, LargeInt second){
	  boolean fBig = greaterList(first.numbers, second.numbers);
	  LargeInt result = new LargeInt();
	  if (fBig)
		  result.numbers = multiplyLists(first.numbers, second.numbers);
	  else
		  result.numbers = multiplyLists(second.numbers, first.numbers);
	  if (first.sign != second.sign)
		  result.setNegative();
	  return result;
  }
  ...
  private static SpecializedList multiplyLists(SpecializedList larger, SpecializedList smaller)
// Precondition: larger >= smaller
//
// Returns a specialized list that is a byte-by-byte sum of the two 
// argument lists
  {
	  int smallerLength = smaller.size();
	  int numIter;
	  SpecializedList temp;

	  SpecializedList result = new SpecializedList(); result.addFront((byte) 0);

	  smaller.resetBackward();

	  for (int count = 1; count <= smallerLength; count++)
	  {
		  numIter = smaller.getPriorElement();
		  temp = new SpecializedList(); temp.addFront((byte) 0);
		  for (int i = 1; i <= numIter; i++){
		  if (greaterList(larger, temp))
			  temp = addLists(larger, temp);
		  else
			  temp = addLists(temp, larger);
		  }
		  for (int c = 1; c < count; c++)
			  temp.addEnd((byte) 0);
		  if (greaterList(temp, result))
			  result = addLists(temp, result);
		  else
